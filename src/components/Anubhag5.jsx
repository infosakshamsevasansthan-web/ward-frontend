...
import { 
  ...
  Home, Users, Trophy, Film, Music, Library, 
  BookOpen, Heart, Shield, Info, MapPin, Search, ArrowRight 
} from 'lucide-react';
...
const Anubhag5 = () => {
  ...
  const categories = [
    { id: "5.1", name: "कामकाजी महिला छात्रावास (क्षमता कोष्ठक में)", icon: <Home /> },
    { id: "5.2", name: "वृद्धाश्रम (ओल्ड एज होम)", icon: <Users /> },
    { id: "5.3", name: "स्टेडियम", icon: <Trophy /> },
    { id: "5.4", name: "सिनेमा हॉल / मल्टीप्लेक्स", icon: <Film /> },
    { id: "5.5", name: "ऑडिटोरियम / सामुदायिक भवन", icon: <Music /> },
    { id: "5.6", name: "सार्वजनिक पुस्तकालय", icon: <Library /> },
    { id: "5.7", name: "सार्वजनिक वाचनालय (रीडिंग रूम)", icon: <BookOpen /> },
    { id: "5.8", name: "अनाथालय / बालगृह", icon: <Heart /> },
    { id: "5.9", name: "बाल सुधार गृह / अवलोकन गृह", icon: <Shield /> },
    { id: "5.10", name: "विशिष्ट दत्तक ग्रहण संस्था", icon: <Users /> },
    { id: "5.11", name: "महिलाओं के लिए वन स्टॉप सेंटर", icon: <Shield /> },
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/api/get-ward-data/${ward}/5`);
        if (res.data && res.data.data) {
          setRows(res.data.data.rows);
          setSource(res.data.data.infoSource || "");
          setSaveStatus(res.data.status);
        }
      } catch (err) { console.log("New session for section 5"); }
    };
    if (ward) fetchData();
  }, [ward]);
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.post('http://localhost:5000/api/save-ward-data', {
        ward_no: ward, section_no: 5, data: { rows, infoSource }, status: currentStatus
      });
      setSaveStatus(currentStatus);
      alert(isComplete ? "अनुभाग-5 पूर्ण सुरक्षित!" : "डेटा सुरक्षित हुआ (लंबित)");
    ...
    <div className="max-w-5xl mx-auto pb-32 pt-6 px-4 font-sans antialiased bg-slate-50/50">
      ...
      <nav className="flex items-center justify-between mb-8 sticky top-4 z-[100] bg-white/80 backdrop-blur-xl p-4 rounded-[2rem] shadow-xl shadow-slate-200/50 border border-white/20">
        ...
        <div className="text-center">
            <h2 className="text-xs font-black text-purple-600 uppercase tracking-[0.2em] mb-0.5">Section 05</h2>
      ...
      {/* Hero Banner */}
      <header className="relative mb-12 p-10 bg-purple-600 rounded-[3rem] overflow-hidden shadow-2xl shadow-purple-200">
          ...
          <div className="relative z-10 flex items-center gap-8 text-white">
              <div className="p-6 bg-white/20 rounded-[2rem] backdrop-blur-md shadow-inner">
                  <Music size={48} strokeWidth={2.5}/>
              </div>
              <div>
                  <h1 className="text-3xl font-black tracking-tight mb-2 uppercase italic leading-none">सामाजिक, मनोरंजन एवं सांस्कृतिक सुविधाएं</h1>
                  <p className="text-purple-100 font-medium max-w-md leading-relaxed opacity-90 text-sm">वार्ड की सामाजिक एवं मनोरंजन सुविधाओं का विवरण दर्ज करें।</p>
              </div>
          </div>
      </header>
      ...
      <section className="mt-12">
          <div className="bg-white p-10 rounded-[3.5rem] shadow-xl border border-slate-100">
              <div className="flex items-center gap-4 mb-8">
                  <div className="p-3 bg-purple-100 text-purple-600 rounded-2xl"><Info size={24}/></div>
              ...
              <input 
                ...
                placeholder="उदा. वार्ड सर्वे, नगर निगम रिकॉर्ड आदि..."
                className={`w-full p-6 bg-slate-50 border-2 rounded-[2rem] outline-none transition-all font-bold text-slate-700
                  ${!infoSource ? 'border-rose-100 focus:border-rose-500 shadow-inner' : 'border-slate-50 focus:border-purple-500 bg-white shadow-lg shadow-purple-100/20'}`}
              />
          </div>
      </section>
...
// --- Modern Card Component (Same high-end logic) ---
const ModernFacilityCard = ({ cat, data, onChange, index }) => {
  const showNearestFields = data.gov_avail === "2" && data.pvt_avail === "2";

  return (
    <div className={`relative bg-white rounded-[4rem] border-2 transition-all duration-700
      ${showNearestFields ? 'border-rose-500 shadow-2xl shadow-rose-200/50 scale-[1.02]' : 'border-white shadow-xl shadow-slate-200/50 hover:border-purple-100 hover:-translate-y-1'}`}>
      ...
      <div className="p-10 space-y-12">
        ...
        <div className="flex items-start gap-6 border-b border-slate-50 pb-8">
            ...
            <div className="pt-2">
                <span className="text-[10px] font-black text-purple-500 uppercase tracking-widest bg-purple-50 px-3 py-1 rounded-full">{cat.id} Category</span>
        ...
        {showNearestFields && (
          <div className="animate-in slide-in-from-bottom-10 fade-in duration-500 bg-rose-50/50 p-10 rounded-[3rem] border-2 border-dashed border-rose-200 relative">
            <div className="absolute -top-4 left-10 bg-rose-500 text-white px-6 py-2 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg shadow-rose-200 flex items-center gap-2">
                <MapPin size={14}/> Nearest Facility Details
            </div>
    ...
    <div className="relative">
      <input 
        ...
        className={`w-full p-5 rounded-2xl border-2 outline-none transition-all font-bold text-lg
          ${disabled ? 'bg-slate-100 border-slate-100 text-slate-300' : 'bg-white border-slate-100 focus:border-purple-500 shadow-inner'}`}
    ...
    <div className="relative">
      <select 
        ...
        className={`w-full p-5 rounded-2xl border-2 bg-white outline-none font-black text-lg cursor-pointer transition-all appearance-none
          ${!value ? 'border-rose-100 focus:border-rose-500' : 'border-slate-100 focus:border-purple-500 shadow-md'}`}
      >
        <option value="">- चुनें -</option>
        {options.map(o => <option key={o.v} value={o.v} className="font-bold">{o.l}</option>)}
      </select>
